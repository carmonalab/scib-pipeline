...
def makePartialAnno(adata, label, unknown):
    """
    params:
        adata: adata object
        label: cell type label column
        unknown: percentage of cell labels to hide
    """
    ...
    adata.obs["unknown_"+str(unknown)+"_"+ label] = unassignPortion(adata.obs["unknown_"+str(unknown)+"_"+ label].values, unknown)
    return adata

def makePartialAndShuffledAnno(adata, label,unknown, shuffling):
    """
    params:
        adata: adata object
        label: cell type label column
        unknown: percentage of cell labels to hide
        shuffling: percentage of cell labels to shuffle (only annotated cells)
    """
    adata.obs["unknown_"+str(unknown)+"_"+ label] = adata.obs[label].copy()
    adata.obs["unknown_"+str(unknown)+"_"+ label] = adata.obs["unknown_"+str(unknown)+"_"+ label].cat.add_categories(['unknown'])
    adata.obs["unknown_"+str(unknown)+"_"+ label] = unassignPortion(adata.obs["unknown_"+str(unknown)+"_"+ label].values, unknown)
    #Shuffling
    alteredAnnoCol = "unknown_"+str(unknown)+"_shuffling_"+str(shuffling)+"_"+ label
    adata.obs[alteredAnnoCol] = adata.obs["unknown_"+str(unknown)+"_"+ label].copy()
    adata.obs[alteredAnnoCol] = shufflePortion(adata.obs[alteredAnnoCol].values, shuffling)
    return adata

def makeShuffling(adata, label,shuffling):
    """
    params:
        adata: adata object
        label: cell type label column
        shuffling: percentage of cell labels to shuffle (only annotated cells)
    """
    adata.obs["percentShuffling_"+str(shuffling)+"_"+ label] = adata.obs[label].copy()
    adata.obs["percentShuffling_"+str(shuffling)+"_"+ label] = shufflePortion(adata.obs["percentShuffling_"+str(shuffling)+"_"+ label].values, n)
    return adata


def runPP(inPath, outPath, hvg, batch, rout, scale, seurat, label, unknown, shuffling):
    """
    params:
    ...
    print("Hiding"+ str(unknown) +"% of annotations and shuffling" + str(shuffling) + "% of remaining annotations...")
    adata = makePartialAndShuffledAnno(adata,label, unknown, shuffling)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--percent_shuffling', help='percentage of shuffling in (available) labels used for supervised methods', default=20)
    parser.add_argument('-u', '--percent_unknown', help='percentage of unnassigned cells in labels used for supervised methods', default=15)
    ...
    batch = args.batch
    label = args.label
    shuflling = args.percent_shuffling
    unknown = args.percent_unknown
    hvg = int(args.hvgs)
    rout = args.rout
    seurat = args.seurat
    scale = args.scale

    runPP(file, out, hvg, batch, rout, scale, seurat,label,unknown, shuflling)
