...
```{r}
if (!file.exists("../supervised_tool_analysis/metrics_R.csv")) {
  file.copy("../../supervised_tool_analysis/metrics_R.csv","./")
}

if (!file.exists("../supervised_tool_analysis/metrics.csv")) {
...
ggplot(metrics[grepl(metrics$annotations,pattern = "shuffled|original"),],aes(x = annotations,color=method,y=ASW_label_raw,group = method)) +geom_point()+ geom_line() +  facet_wrap('~task') +
...
ggplot(metricsR[grepl(metricsR$annotations,pattern = "shuffled|original"),],aes(x = annotations,color=method,y=CiLISI,group = method)) +geom_point()+ geom_line() +  facet_wrap('~task') +
  scale_x_discrete(guide = guide_axis(angle = 45))
```
...
## ASW label score
```{r}
ggplot(metrics[grepl(metrics$annotations,pattern = "unknown|original"),],aes(x = annotations,color=method,y=ASW_label_raw,group = method)) +geom_point()+ geom_line() +  facet_wrap('~task') +
  scale_x_discrete(guide = guide_axis(angle = 45))
```
...
```{r}
ggplot(metricsR[grepl(metricsR$annotations,pattern = "unknown|original"),],aes(x = annotations,color=method,y=CiLISI,group = method)) +geom_point()+ geom_line() +  facet_wrap('~task') +
  scale_x_discrete(guide = guide_axis(angle = 45))
```
...
```{r}
ggplot(metrics[grepl(metrics$annotations,pattern = "broad|original")&metrics$task %in% c('immune_cell_hum','tcells_atlas'),],aes(x = annotations,color=method,y=ASW_label_raw,group = method)) +geom_point()+ geom_line() +  facet_wrap('~task') +
...
```{r}
ggplot(metricsR[grepl(metricsR$annotations,pattern = "broad|original")&metricsR$task %in% c('immune_cell_hum','tcells_atlas'),],aes(x = annotations,color=method,y=CiLISI,group = method)) +geom_point()+ geom_line() +  facet_wrap('~task') +
  scale_x_discrete(guide = guide_axis(angle = 45)) + theme(legend.position = "bottom")
```
...
for (t in unique(allMetrics$task)) {
  ...
  plot(ggplot(metricsTask[grepl(metricsTask$annotations,pattern = "unknown|original"),],aes(x = CiLISI, y= ASW_label_raw, color = method, group = method)) +geom_point() + facet_wrap(c("task","annotations"), ncol = 5)+theme(legend.position = "bottom"))
  plot(ggplot(metricsTask[grepl(metricsTask$annotations,pattern = "shuffled|original"),],aes(x = CiLISI, y= ASW_label_raw, color = method, group = method)) +geom_point() + facet_wrap(c("task","annotations"), ncol = 5)+theme(legend.position = "bottom"))
}
```
## Broad annotations
