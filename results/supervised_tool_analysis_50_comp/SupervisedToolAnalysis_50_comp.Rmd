...
#setwd("./results/supervised_tool_analysis_50_comp")


if (!file.exists("./metrics_R.csv")) {
  file.copy("../../supervised_tool_analysis_50_comp/metrics_R.csv","./")
}

if (!file.exists("./metrics.csv")) {
  file.copy("../../supervised_tool_analysis_50_comp/metrics.csv","./")
}
```


It also requires the files
-   `../original_annotations_50_comp/metrics_R.csv`
-   `../original_annotations_50_comp/metrics.csv`

generated with the snakemake pipeline using the config file `configs/test_original_annotations-R4.1.yaml`.
First we copy them in the `results/original_annotations_50_comp` directory (already done in the repository)

#NB: this needs to be generated

```{r}
dir.create("../original_annotations_50_comp/")
if (!file.exists("../original_annotations_50_comp/metrics_R.csv")) {
  file.copy("../../original_annotations_50_comp/metrics_R.csv","../original_annotations_50_comp/metrics_R.csv")
}

if (!file.exists("../original_annotations_50_comp/metrics.csv")) {
  file.copy("../../original_annotations_50_comp/metrics.csv","../original_annotations_50_comp/metrics.csv")
}
```
...
metricsR_unaltered <- read.csv("../original_annotations_50_comp/metrics_R.csv",row.names = 1)
...
metrics_unaltered <- read.csv("../original_annotations_50_comp/metrics.csv",row.names = 1)
...
#Source data
source <- data[,c("task","tool","annotations","ASW_label_raw")] 
source <- source[with(source, order(task, tool, annotations)),]

write.csv(x=source, file="shuffle_50pc_ASW.csv")
```
...
#Source data
source <- data[,c("task","tool","annotations","CiLISI")] 
source <- source[with(source, order(task, tool, annotations)),]

write.csv(x=source, file="shuffle_50pc_CiLISI.csv")
```
...
write.csv(x=source, file="unknown_50pc_ASW.csv")
```
...
write.csv(x=source, file="unknown_50pc_CiLISI.csv")
```
