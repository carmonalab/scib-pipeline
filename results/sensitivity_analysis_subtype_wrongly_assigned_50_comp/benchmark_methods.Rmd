...
#rownames(metricsCiLISIlabelASW) <- gsub("^/", replacement = "", x = rownames(metricsCiLISIlabelASW), perl=T)

# remove noise info in rowname to directly use luecken function
rownames(metricsCiLISIlabelASW) <- gsub(rownames(metricsCiLISIlabelASW), perl=T,
                                        pattern = '/neighbor_shuffled_20_unknown_15|unknown_15_shuffled_20/', replacement = "")

rownames(metricsCiLISIlabelASW) <- gsub(rownames(metricsCiLISIlabelASW), perl=T,
                                        pattern = '_subtype_wrongly_assigned', replacement = "")
...
oriMetrics <- oriMetrics[!oriMetrics %in% c("hvg_overlap","trajectory")]

metrics_or <- metrics[!metrics$method %in% c("seuratrpca_full","semiSupSTACAS_full","STACAS_full","seurat_full","fastmnn_full"),]
metrics_or <- metrics_or[,oriMetrics]



# remove noise info in rowname to directly use luecken function
rownames(metrics_or) <- gsub(rownames(metrics_or), perl=T,
                                        pattern = '/neighbor_shuffled_20_unknown_15|unknown_15_shuffled_20/', replacement = "")
...
write.csv(metrics_or,"unknown_15_shuffled_20/orimetrics/metrics.csv",col.names = T)
...
plotSingleTaskRNA("unknown_15_shuffled_20/orimetrics/metrics.csv",
                  outdir = "unknown_15_shuffled_20/orimetrics/singleTask")
